...
public class Warehouse {
    ...
    private void addToEnd(int id, String name, int stock, int day, int demand) {
        Product newProduct = new Product(id, name, stock, day, demand);//initialize new product
        int endId = id % 10;
        sectors[endId].add(newProduct);
    }
    ...
    private void fixHeap(int id) {
        int endId = id % 10;
        int sectorSize = sectors[endId].getSize();
        sectors[endId].swim(sectorSize);
    }
    ...
    private void evictIfNeeded(int id) {
        int endId = id % 10;
        if (sectors[endId].getSize() == 5){
            sectors[endId].swap(1, 5);
            sectors[endId].deleteLast();
            sectors[endId].sink(1);
        }
    }
    ...
    public void purchaseProduct(int id, int day, int amount) {
        for(int i = 1; i <= sectors[id % 10].getSize(); i++){
            if(sectors[id % 10].get(i).getStock()  - amount >= 0 && sectors[id % 10].get(i).getId() == id){
    ...
    public static void main(String[] args){
        ...
        while (count < lines && StdIn.hasNextLine()){
    ...
    public void betterAddProduct(int id, String name, int stock, int day, int demand) {
        ...
        if (sectors[endId].getSize() < 5) { 
            ...
            while (edge < 10) { 
                ...
                if (sectors[edge].getSize() < 5) {
                    addToEnd(edge, name, stock, day, demand);
                    fixHeap(edge);
                    break;
                } 
